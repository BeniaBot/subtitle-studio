...
# ---------- קובצי SRT מהעולם האמיתי ----------
# הקטגוריה הזאת היא תקלת השקט הגרועה ביותר בתוכנה: קובץ תקין נפתח ריק,
# ואז ImportSubs מודיע למשתמש ש"נראה שזה קובץ טקסט רגיל" - כלומר מאשים
# אותו בקובץ שלם לגמרי. כל שורה כאן היא צורה שקיימת בקבצים אמיתיים.

Write-Host 'קובצי SRT מהעולם האמיתי'
$ST2 = [Reflection.BindingFlags]'NonPublic,Public,Static'
$ps = $fmt.GetMethod('ParseSrt', $ST2)
function P1($srt) { return $ps.Invoke($null, (Pack ([string]$srt))) }
function CueText($c, $i) { return $cueT.GetField('Text').GetValue($c[$i]) }

$c = P1 "1`n00:00:01,000 --> 00:00:02,000`n<i>שלום</i>`n"
Eq 'תגיות נטוי מוסרות' (CueText $c 0) 'שלום'
$c = P1 "1`n00:00:01,000 --> 00:00:02,000`n<font color=`"#f00`">שלום</font>`n"
Eq 'תגית font מוסרת' (CueText $c 0) 'שלום'
$c = P1 "1`n00:00:01,000 --> 00:00:02,000`n{\an8}שלום`n"
Eq 'תגית ASS מוסרת' (CueText $c 0) 'שלום'
$c = P1 "00:00:01,000 --> 00:00:02,000`nשלום`n"
Eq 'בלי מספר סידורי' $c.Count 1
$c = P1 "5`n00:00:01,000 --> 00:00:02,000`nא`n`n9`n00:00:03,000 --> 00:00:04,000`nב`n"
Eq 'מספור לא רציף' $c.Count 2
$c = P1 "1`n00:00:01,000 --> 00:00:02,000`nא`n2`n00:00:03,000 --> 00:00:04,000`nב`n"
Eq 'בלי שורה ריקה מפרידה' $c.Count 2
$c = P1 "1`n0:00:01,000 --> 0:00:02,000`nשלום`n"
Eq 'שעה חד-ספרתית' $c.Count 1

# זה נשבר עד עכשיו והחזיר אפס
$c = P1 "1`n00:00:01 --> 00:00:02`nשלום`n"
Eq 'בלי אלפיות שנייה' $c.Count 1
if ($c.Count -eq 1) {
    Eq 'והזמנים נכונים' ($cueT.GetField('Start').GetValue($c[0])) 1000
}

$c = P1 "1`n00:00:01,000 --> 00:00:02,000  X1:100 X2:200 Y1:10 Y2:20`nשלום`n"
Eq 'קואורדינטות בסוף שורת הזמן' $c.Count 1
$c = P1 ([char]0xFEFF + "1`n00:00:01,000 --> 00:00:02,000`nשלום`n")
Eq 'BOM בתחילת הקובץ' $c.Count 1
$c = P1 "1`n00:00:01,000 --> 00:00:02,000`n- שלום`n- להתראות`n"
Eq 'מקף דובר נשמר' (CueText $c 0) "- שלום`n- להתראות"
